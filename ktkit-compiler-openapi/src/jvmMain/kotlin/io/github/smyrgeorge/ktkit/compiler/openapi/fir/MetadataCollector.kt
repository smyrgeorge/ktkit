...
class MetadataCollector(
    ...
    override fun check(expression: FirFunctionCall) {
        store.collectOpenApiInfo(expression.annotations, expression)
    ...
    private fun Map<String, FirExpression>.strings(name: String): List<String> =
        this[name]?.elements(name)?.map { element ->
            element.constStringOrNull() ?: throw UnsupportedValue("'$name' contains a non-string element")
        } ?: emptyList()

    private fun FirExpression.elements(name: String): List<FirExpression> = when (this) {
        is FirVarargArgumentsExpression -> arguments.map { it.unwrapArgument() }
        is FirCall -> argumentList.arguments.map { it.unwrapArgument() }
