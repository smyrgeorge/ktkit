...
abstract class AbstractRestHandler(
     ...
     *
     * This method is normally not implemented by hand: the `io.github.smyrgeorge.ktkit.openapi`
     * compiler plugin generates an override for every concrete handler at compile time by statically
     * analyzing [uri] and [routes]. A hand-written override is respected (the plugin skips such
     * handlers). The fragment contains the `paths` and `components` of this handler only; the
     * fragments of all registered handlers are merged and served by
